...
with tab1:
    ...
    with st.form(key=f"expense_form_{selected_date}"):
        ...
        for i in range(5):
            ...
            column1, column2, column3 = st.columns(3)
            with column1:
                amount_input=st.number_input(label="Amount",min_value=0.0,step=1.0, value=amount, key=f"amount_{selected_date}_{i}", label_visibility="collapsed")
            with column2:
            ...
            with column3:
                notes_input= st.text_input(label="Note", value=notes, key=f"notes_{selected_date}_{i}", label_visibility="collapsed")
